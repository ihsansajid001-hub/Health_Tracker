  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // Redirect to onboarding with username
        window.location.href = `/onboarding?username=${username.toLowerCase()}`;
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('not enabled')) {
          throw new Error('Google sign-up is not enabled yet. Please enable it in Supabase Dashboard → Authentication → Providers → Google');
        }
        throw error;
      }
    } catch (err: any) {
          ...
          <div className="relative">
            ...
            <div className="relative z-10 w-[280px] h-[580px] bg-gray-900 rounded-[3rem] p-3 shadow-2xl">
              <div className="w-full h-full bg-white rounded-[2.5rem] overflow-hidden">
                {/* Phone Screen Content - Mobile Signup Preview */}
                <div className="h-full bg-gradient-to-br from-blue-500 to-indigo-600 flex flex-col items-center justify-center p-6">
                  ...
                  <div className="w-full bg-white rounded-2xl p-6 shadow-xl">
                    <div className="space-y-3">
                      ...
                      <div className="h-8 bg-gray-100 rounded-lg"></div>
                      <div className="h-8 bg-gray-100 rounded-lg"></div>
                      <div className="h-8 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-lg"></div>
                    </div>
                  </div>
