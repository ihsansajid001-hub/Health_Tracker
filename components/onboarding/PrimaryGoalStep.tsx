...
export default function PrimaryGoalStep({ data, setData, onNext, onBack }: Props) {
  const goals = [
    ...
    { value: 'muscle_gain', icon: '💪', label: 'Muscle Gain', color: 'from-orange-500 to-cyan-500' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex space-x-3 pt-4">
        ...
        <button onClick={onNext} disabled={!data.primary_goal} className="flex-1 py-3 bg-gradient-to-r from-orange-500 to-purple-600 text-white rounded-lg font-semibold disabled:opacity-50">Next →</button>
      </div>
    </div>
  );
}
